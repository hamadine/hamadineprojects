// app-corrige.js — version corrigée et structurée
...
const envoyerMessage = () => {
  ...
  // Recherche dans histoire et multimédia
...
const rechercherDansHistoireEtMedia = (clean) => {
  ...
  for (const [trigger, key] of Object.entries(trig)) {
    if (clean.includes(nettoyerTexte(trigger))) {
      ...
      if (match) {
        ...
        if (match.image) {
          contenu += `<br><img src="${match.image}" alt="Illustration" style="max-width:100%;margin-top:10px;">`;
        }
        if (match.video) {
          contenu += `<br><video controls width="100%" style="margin-top:10px;">
  ...
  if (results.length) {
    const bloc = results.map(d => {
      let cont = `<strong>${escapeHTML(d.titre)}</strong><br>${escapeHTML(d.contenu)}`;
      if (d.image) cont += `<br><img src="${d.image}" alt="Image" style="max-width:100%;margin-top:5px;">`;
      if (d.video) cont += `<br><video controls width="100%" style="margin-top:10px;">
  ...
  // Suggestions automatiques (si aucun résultat)
  ...
  if (suggestions.length) {
    ...
    return afficherMessage('bot', `❓ Aucune réponse exacte. Suggestions :<ul>${list}</ul>`);
  }
