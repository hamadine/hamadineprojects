...
function envoyerMessage() {
  ...
  if (remerciements.includes(message)) {
    afficherMessage('bot', botData.remerciementsRandom || "Merci à vous !");
    return;
  }
...
function traiterRecherche(message, reponseMot, inconnu) {
  setTimeout(() => {
    ...
    if (resultats.length) {
      ...
      afficherMessage('bot', `${reponseMot || window.reponseBot}<br><br>${suggestions}`);
    } else {
      afficherMessage('bot', inconnu || "Mot non trouvé. Merci de votre contribution !");
    }
  }, 300);
}
